"""
nav_to_czml takes in nav data from various aricrafts (dc-8) of CPHEX camapign.
...
def data_pre_process(bucket_name="ghrc-fcx-field-campaigns-szg", field_campaign = "Cpexaw", input_data_dir = "instrument-raw-data", output_data_dir = "instrument-processed-data", instrument_name = "nav_dc8"):
    """
    ...
    Args:
        ...
        field_campaign (str, optional): name of field campaign. Case sensitive. Defaults to "CPHEX-AW".
    ...
    for obj in s3bucket.objects.filter(
            Prefix=f"{field_campaign}/{input_data_dir}/{instrument_name}/CPEX"):
        keys.append(obj.key)
    ...
    for infile in result:
        ...
        local_file_dir = f"/tmp/cpexaw/raw/nav/"
        if not os.path.exists(local_file_dir):
            os.makedirs(local_file_dir)
        local_file_path = f"{local_file_dir}{file_name}.ict"
        s3_client.download_file(bucket_name, infile, local_file_path)

        year = 2021 # when the campaign took place.
        reader = FlightTrackReader(year)
        reader.read_csv(local_file_path) # icartt needs path to file. not file object.
...
def dc8():
    ...
    field_campaign = "CPEX-AW" # need to change the casing. Also the s3 url creation.
